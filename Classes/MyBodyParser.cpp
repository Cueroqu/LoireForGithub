            ...
            if (0 == strcmp(name.c_str(), bodies[i]["name"].GetString()))
            {
                ...
                if (bd.IsObject())
                {
                    ...
                    for (int i = 0; i<polygons.Size(); ++i)
                    {
                        ...
                        body->addShape(PhysicsShapePolygon::create(points, pcount, PhysicsMaterial(0.50f, 5.0f, 0.0f)));
                        delete [] points;
                    }
                }
                else
...
std::vector<cocos2d::Vec2 * > MyBodyParser::get_shape_from_json
(cocos2d::Node *pNode, const std::string &name, std::vector<int> & nums)
{
    rapidjson::Value &bodies = doc["rigidBodies"];
            ...
            if (!strcmp(name.c_str(), bodies[i]["name"].GetString())) {
                ...
                if (bd.IsObject()) {
                    ...
                    for (int j = 0; j < polygons.Size(); ++j) {
                        ...
                        nums.push_back(pcount);
                    }
                }
                else
                {
                    MessageBox(name.c_str(), "Body Not Found");
                }
                break;
            }
        }
